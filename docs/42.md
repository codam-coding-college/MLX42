...
- `Not maintained`: The library is practically dead, and it's unclear who is available to fix the bugs. It hasn't had any changes in a long time, and student pull requests on Github for the X11 version go unnoticed. Issues are ignored.
...
The main goal of MLX42 is to empower students and instructors by giving them the ability to maintain and fix bugs, instead of leaving their complaints unaddressed. By being open-source, students can explore the code and submit pull requests.
...
Students do not need to understand how to use `cmake`, as building the library requires only two shell commands. Instructions on how to build the library are provided to them.
...
A: No, there are 100% breaking changes, and changing from miniLibX to MLX42 will not be a simple drag-and-drop replacement. That was not the idea, as it would be impossible to fix some of the problems present in miniLibX.
