...
A: No, there are 100% breaking changes, and changing from miniLibX to MLX42 will not be a simple drop-in replacement. That was not the idea, as it would be impossible to fix some of the problems present in miniLibX.
...
Q: **_"Do we need update translations or subjects, etc??"_**

A: Regarding the subjects, as long as 42 keeps being stubborn about adopting the library there's little room for change in this regard. However the good news is that it basically requires zero effort besides updating the links on the intra and maybe changing the name referenced inside the pdf's. There is really nothing to necessary to change besides minor things and to adapt these changes would literally just require at most a day of effort.
